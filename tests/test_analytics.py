...
class TestGetTopPosts:
    ...
    def test_limit_respected(self, analytics_db):
        from analysis.analytics import get_top_posts
        _seed_basic_data(analytics_db)
        df = get_top_posts(topic="All", limit=2)
        assert len(df) <= 2
